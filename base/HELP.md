...
Class-specific (write in ONLY if assembled): the class's unit-of-work flow in its own phrase ("take a task" / "answer a question" / "make a decision"); upcoming and overdue commitments from the calendar; working with your own code ("make a script/site/bot"); reverse search by sources and the evidence map — only with the claim graph.>>
