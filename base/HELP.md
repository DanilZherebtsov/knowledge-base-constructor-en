...
Class-specific (write in ONLY if assembled): the class's unit-of-work flow in its own phrase ("take a task" / "answer a question" / "make a decision"); upcoming and overdue commitments from the calendar; working with your own code ("make a script/site/bot"); reverse search by sources and the evidence map — only with the claim graph; work on visual things with the `design` mechanic — "**make a presentation / landing page / cover**" (I'll first check who it's for and why, show you a few directions to choose from, then show you the built result; I keep the style consistent myself).>>
