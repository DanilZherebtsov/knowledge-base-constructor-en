...
- **Saving — always marked with the slice.** New findings — into the shared wiki via a regular [ingest](ingest.md) (with `sources:`, the human confirms); draft analysis — into the class's working layer; a decision — an ADR in `decisions/`. **While working as a role, every write to the wiki carries the role's slice marking:** a new page — under the role's prefix and with its tag in `tags:`; appended a finding to an existing page — check the marking there, missing → add it and refresh `updated:`. Slice marking is not a one-off event at role creation (step 4) but part of every write: otherwise the slice goes stale with the very first role finding, and next session the role cannot see its own output. In the closing report to the human, name what was written and under which marking.
- **Shared page, role finding** (appended to a journal page, or where other roles' material already sits): the role's tag goes on the page anyway. A slice is navigation ("there is role material here"), not ownership; neighboring entries come along into the slice — cheaper than losing the finding.
