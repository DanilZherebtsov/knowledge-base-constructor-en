...
- **`archive/`** — material from `output/` and `tmp/` that has aged out and is not worth deleting; not created empty, it appears at the first retirement ([lint.md](lint.md)). Under git — add `archive/` to `.gitignore` at creation time, before the first file is moved in.
