...
**What it does (1 line):** gives a class **full competence over its own code** — a home for the code (essentially a root code folder), the "code beats the wiki" rule, the **execution cycle** (task statement → independent review gate by subagents → implementation → repeat gate by running tests → report; loop on errors; blocker escalation), secrets-in-code and runtime-data discipline, **long-running-operation discipline** (progress to disk as it goes, resumption instead of a restart, cleanup of temporary artifacts on completion), **the discipline of extracting data from a foreign format** (a boundary sample before the bulk run, a rejects journal instead of a silent empty value, reconciliation of the total against the source), reproducible deployment, a **reconnaissance pass over code that arrives ready-made** (an inherited base is described into the wiki before the first task; an empty impact lens is a blocker on the statement, not a clean gate), and an on-demand offer of the "edits/deploys" role pair. Writes code **from scratch or on top of existing code**. Does **not** claim the central wiki type; composes with any class lifecycle (paired with `spec-lifecycle` for saas: the spec is the unit of work, this mechanic is the execution).
...
6. **Pointer in "Wiki: page types and operations" (the always-on entry for both triggers).** After the domain unit-of-work flow add the line: "**Code ownership and the execution cycle** — the code folder (source of truth on disk), secrets discipline in code, the code-writing cycle, product roles; **working code arrives OR it is decided to build code from scratch → first read [methodology/software-engineering.md](methodology/software-engineering.md)**." This gives the "from scratch" trigger (no ingest) an always-on entry point.
...
2. Leave the OWNED-CODE slot in `ingest.md` in its base variant ("if the project starts seriously owning code — offer to attach the `software-engineering` mechanic; the attach procedure — `lint.md`, the "Attach a capability the project doesn't have" section").
...
3. **Source of the mechanic's files**, if the constructor scaffolding is already removed: the upstream mirror (the same channel that maintenance/lint uses for SHA version checks). If local scaffolding is in place (or a copy of the mirror sits next to the project) — take from it. The fetch procedure and the degradation floor ("couldn't fetch — say so plainly; never reconstruct from memory") — `base/methodology/lint.md`, the "Attach a capability the project doesn't have" section.
