...
2. **Task statement gate — ≥4 independent reviews (before implementation), in parallel:**
   ...
   Findings are consolidated → the statement is revised (the spec is mutable while `active`) → the gate **repeats** until the review is clean. **The gate scales with risk:** an irreversible or wide-impact change (flagged by the impact reviewer) gets a strengthened gate — more verifiers and a dedicated adversarial round aimed at *disproving* that the change is safe; a routine local edit — the base 3+1.
