...
7. **`page-conventions.md` → the atomicity section, "Citations with localization", and the CITE-LOC slot.** Add the claim limit of ≤ 100 lines (split via `refines`) and the citation-localization block with formats (Piece 4). Fill the **CITE-LOC** slot with the rule "**Citations carry localization.** A source reference includes the specific place (page / paragraph / timecode)".
