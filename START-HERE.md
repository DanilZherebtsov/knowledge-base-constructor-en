...
The assembled project `CLAUDE.md` (from `base/`, filled in) goes to the root, **replacing the constructor's root `CLAUDE.md` launcher**. Then delete all the constructor tooling: `base/`, `mechanics/`, `presets/`, `migrations/`, `START-HERE.md`, `ASSEMBLY.md`, `mechanics-catalog.md`, `README.md`, `CHANGELOG.md`, `versions.json` (the root launcher is already overwritten). **The list is a hint, not the source of truth: check against the "Architecture" tree in the assembled `CLAUDE.md`** — what stays is exactly what is listed there (plus the hidden `.claude/`); everything else in the root is scaffolding. The constructor is scaffolding, not part of the building.

**Nothing to delete with — do not substitute something else for the cleanup.** No deletion tool, a read-only sandbox — **do not create a dumping-ground folder** (`_to_delete` and the like: the scaffolding stays in the project under another name, and the human is later left guessing what it is and whether it can go). Instead — name in plain words what is left to remove, and give one ready command with the exact list, for the human to run themselves.
...
> ✓ (when there is nothing to delete with) "The project is assembled, the structure is in place. What's left are the temporary files that helped assemble it — one command removes them: `rm -rf base mechanics presets migrations START-HERE.md ASSEMBLY.md mechanics-catalog.md README.md CHANGELOG.md versions.json`"
