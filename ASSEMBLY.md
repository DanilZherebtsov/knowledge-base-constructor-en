...
3. **Substitute the scalar slots** from the manifest into base:
   ...
   - Slots marked "otherwise empty" that no wired-in mechanic fills (`CITE-LOC`/`DEADLINE-CHECK` in a class without `claim-graph`/`decision-lifecycle`) → remove the marker (resolve to empty).
